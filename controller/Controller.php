...
// creo il descrittore della vista
                    ...
                    case 'registrazione':
                        ...
                        if(isset($request['marcaCartaCredito'])){
                            if (!filter_var($request['marcaCartaCredito'], FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => '/[a-zA-Z]{4,15}/')))) {
                                ...
                                $ajaxItem->setMessaggio('Inserisci una marca con lunghezza compresa fra 4 e 15 lettere');
    ...
    protected function login($pageContent, $username, $password) {
        ...
        if (isset($user)) {
            ...
            $pageContent->setMessaggioErrore("Utente sconosciuto o password errata");
            $this->showLoginPage($pageContent);
        }//end else
    }
    ...
    protected function creaFeedbackUtente(&$message, $pageContent, $okMessage) {
        if (count($message) > 0){
            ...
            $error = $error . "\n</ul>\n";
            $pageContent->setMessaggioErrore($error);
        }//end if
        else
            $pageContent->setMessaggioConferma($okMessage);
    }
